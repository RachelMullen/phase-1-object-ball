...
function teamColors(name){
    if (home.teamName === name)
        return home.colors
 }
 else if(away.teaName === name) {
    return away.colors;
} else {
    return;
}
...
function playerNumbers(name) {
    let team = selected(name);
    let result = [];
    for(let t in team)[
        result.push(team[t].number)
    ] 
}

//Helper function aka callback function
function selected(name){
    return home.teamName == name ? home.player : away.players;
}
...
function playerStats(name)
    ...
    for (let players.points = 
    let value = numPointsScored[points];
